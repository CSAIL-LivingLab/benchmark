...
public class TerminatorThread extends Thread {
	...
	static final Logger logger = Logger.getLogger(TerminatorThread.class);
  ...
  public TerminatorThread(long maxExecutionTime, Vector<Thread> threads, 
    ...
    logger.warn("Maximum execution time specified as: " + maxExecutionTime + " secs");
  }
  ...
  public void run() {
    try {
      Thread.sleep(maxExecutionTime * 1000);
    	...
    	logger.warn("Could not wait until max specified time, TerminatorThread interrupted.");
      return;
    }
    logger.warn("Maximum time elapsed. Requesting stop for the workload.");
    workload.requestStop();
    logger.warn("Stop requested for workload. Now Joining!");
    for (Thread t : threads) {
      while (t.isAlive()) {
        try {
          t.join(waitTimeOutInMS);
          if (t.isAlive()) {
        	  logger.warn("Still waiting for thread " + t.getName() + " to complete. " +
